                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mt-2">
                                            ...
                                            <input type="text" class="form-control form-control-lg" id="name" placeholder="{{__('enter')}}{{__('name')}}" name="name" value="{{ old('name',$manager->name) }}">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mt-2">
                                            <label for="email">{{__('email')}}</label>
                                            <input type="email" class="form-control form-control-lg" id="email" placeholder="{{__('enter')}}{{__('email')}}" name="email" value="{{ old('email',$manager->email) }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <select class="form-control form-control-lg" id="floatingSelectGrid" aria-label="{{ __('role') }}" name="role">
                                                ...
                                                @foreach($roles as $role)
                                                    <option value="{{$role->id}}" @if(old("role",$manager->role->id) == $role->id ) selected @endif >{{$role->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @error("role")
