            ...
            <div class="dropdown d-inline-block language-switch">
                <button type="button" class="btn header-item waves-effect" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    @switch(app()->getLocale())
                        @case('ar')
                            <img src="{{ URL::asset('/assets/images/flags/ar.jfif')}}" alt="Header Language" height="16"> <span class="align-middle"></span>
                            @break
                        @default
                            <img src="{{ URL::asset('/assets/images/flags/us.jpg')}}" alt="Header Language" height="16"> <span class="align-middle"></span>
                    @endswitch
                </button>
                <div class="dropdown-menu dropdown-menu-end flex-column align-items-start ps-3 gap-2">
                    <!-- item-->
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <li>
                            <a rel="alternate" hreflang="{{ $localeCode }}" href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}">
                                @if($localeCode == 'en')
                                    <img src="{{ URL::asset('assets/images/flags/us.jpg')}}" alt="user-image" class="me-1" height="12"> <span class="align-middle">{{__('english')}}</span>
                                @elseif($localeCode == 'ar')
                                    <img src="{{ URL::asset('assets/images/flags/ar.jfif')}}" alt="user-image" class="me-1" height="12"> <span class="align-middle">{{__('arabic')}}</span>
                                @else
                                    {{ $properties['native'] }}
                                @endif
                            </a>
            ...
            <div class="dropdown d-inline-block">
                <button type="button" class="btn header-item waves-effect" id="page-header-user-dropdown"
                    ...
                    @if(Auth::guard('admin')->user()->image)
                        <img class="rounded-circle header-profile-user" src="{{ Auth::guard('admin')->user()->image->url  }}" alt="{{Auth::guard('admin')->user()->image->filename}}">
                    @else
                        <img class="rounded-circle header-profile-user" src="{{ URL::asset('/assets/images/users/avatar-4.jpg') }}" alt="Header Avatar">
                    @endif
                    <span class="d-none d-xl-inline-block ms-1 fw-medium font-size-15">{{Str::ucfirst(Auth::guard('admin')->user()->name)}}</span>
                    <i class="uil-angle-down d-none d-xl-inline-block font-size-15"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-end">
                    <!-- item-->
                    <a class="dropdown-item" href="{{route('profile.index')}}"><i class="uil uil-user-circle font-size-18 align-middle text-muted me-1"></i> <span class="align-middle">{{__('view_profile')}}</span></a>
{{--                    <a class="dropdown-item d-block" href="#"><i class="uil uil-cog font-size-18 align-middle me-1 text-muted"></i> <span class="align-middle">@lang('translation.Settings')</span> <span class="badge bg-soft-success rounded-pill mt-1 ms-2">03</span></a>--}}
                    <a class="dropdown-item" style="cursor: pointer" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="uil uil-sign-out-alt font-size-18 align-middle me-1 text-muted"></i> <span class="align-middle">{{__('sign_out')}}</span></a>
